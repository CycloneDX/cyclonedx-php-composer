...
Possible evidence:
- mandatory: composer config/manifest file (e.g. `composer.json` file )
- either: an actual composer setup (e.g. `vendor` directory, after running `composer install` on your project)
- or: a working composer lock file (e.g. `composer.lock` file)
